...
def drawRationalBezier(cp, w, N):
    ...
    for t in np.linspace(0, 1, N):
        ...
        for i in range(0, number_cp):
            ## Calcurate the numerator c on Bezier curve from control point cp, w and etc... ##
            c += w[i] * (combination(n, i) * (t ** i) * ((1 - t) ** (n - i)))
            ## Calcurate the denominator w on Bezier curve from weight list w and etc... ##
            sum_weight += w[i] * cp[i] * (combination(n, i) * (t ** i) * ((1 - t) ** (n - i)))
            a = sum_weight / c
        ## Specifying a vertex ##
...
def display():
    ...
    # Draw Bezier curve
    cp = np.array([[0.1, 0.1], [0.1, 0.9], [0.5, 0.1],
