...
class Spotlights:
    def __init__(self, pos, color, I0, theta, phi, init_t):
        ...
        self.dir = np.array([math.sin(3*(self.init_t))*math.cos(self.init_t)/2, self.pos[1] -
                            0.5, math.sin(3*(self.init_t))*math.sin(self.init_t)/2]) - self.pos

        self.color = color
        self.I0 = I0            # Intensity
        # Angle for outer-corn range (radian: 0 < theta < pi)
        self.theta = theta
        # Angle for inner-corn range (radian: 0 < phi < pi)
        self.phi = phi

    def update(self, t):
        self.dir = np.array([math.sin(3*(t+self.init_t))*math.cos(t+self.init_t)/2, self.pos[1] -
                            0.5, math.sin(3*(t+self.init_t))*math.sin(t+self.init_t)/2]) - self.pos

    def drawLightDirection(self):
        glColor3f(self.color[0], self.color[1], self.color[2])
        glBegin(GL_LINES)
        glVertex3f(self.pos[0], self.pos[1], self.pos[2])
        glVertex3f(self.dir[0]+self.pos[0], self.dir[1] +
                   self.pos[1], self.dir[2]+self.pos[2])
        glEnd()

    def calcColorFromLight(self, target_pos):
        colored_by_light = np.array([0.0, 0.0, 0.0])
        # Vector from light to target
        L2T = target_pos - self.pos
        distance = np.linalg.norm(L2T)
        L2T = L2T/distance if distance > 0 else 0           # Normalize L2T
        # Normalize the direction of the light source
        light_direction = self.dir/np.linalg.norm(self.dir)

        N = np.array([0, 1, 0])
        Lambert = np.dot(N, -L2T)

        ## The condition formula that the angle between L2T and light_direction is less than or equal to phi/2 ##
        # if L2T and light_direction <= g_phi / 2:
        if np.arccos(np.dot(L2T, light_direction)) <= self.phi / 2:
            colored_by_light = Lambert*self.I0*np.array(self.color)/distance**2

        ## The condition formula that the angle between L2T and light_direction is less than or equal to theta/2 and more than phi/2 ##

        if self.phi / 2 <= np.arccos(np.dot(L2T, light_direction)) <= self.theta / 2:
            decay = (np.dot(L2T, light_direction) - np.cos(self.theta/2)
                     )/(np.cos(self.phi/2) - np.cos(self.theta/2))
            colored_by_light = Lambert*decay*self.I0 * \
                np.array(self.color)/distance**2
    ...
    for j in range(0, N):
        for i in range(0, M):
            ...
            for k in range(len(spotlight)):
                ## Call the function in Spotlights class to calculate the color c00 at p00. ##

                c00 += spotlight[k].calcColorFromLight(p00)
                ## Call the function in Spotlights class to calculate the color c10 at p10. ##

                c10 += spotlight[k].calcColorFromLight(p10)
                ## Call the function in Spotlights class to calculate the color c11 at p11. ##

                c11 += spotlight[k].calcColorFromLight(p11)
                ## Call the function in Spotlights class to calculate the color c01 at p01. ##

                c01 += spotlight[k].calcColorFromLight(p01)
...
def main():
    ...
    color2 = [0.0, 1.0, 0.0]
    sl2 = Spotlights(light_pos, color2, 5.0, math.pi /
                     4, math.pi/15, 2*math.pi/3)
    spotlight.append(sl2)
    color3 = [0.0, 0.0, 1.0]
    sl3 = Spotlights(light_pos, color3, 5.0, math.pi /
                     4, math.pi/15, 4*math.pi/3)
