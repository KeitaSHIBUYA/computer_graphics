...
def calcNewDragonCurvePoint(theta, p1, p2):
    ...
    ## Calculate new vertex after_p2 using rotation_matrix and before_p2 ##
    factor = 1 / np.sqrt(2)
    # print(factor)

    after_p2 = np.dot(rotation_matrix,before_p2) * factor + p1
    # print(after_p2)
...
def drawDragonCurve(generation, point1, point2):
    if generation == 0:
        ...
        ## Calculate new vertex using calcNewDragonCurvePoint as let point1 be a rotation center ##
        newpoint = calcNewDragonCurvePoint(np.radians(45),point1,point2)
        # print(newpoint)

        ## This's a recursive program! ##
        drawDragonCurve(generation - 1, point1 , newpoint)
...
def display():
    ...
    # drawDragonCurve(9, [0.25, 0.5], [0.75, 0.5])   # for debug
