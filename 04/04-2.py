        ...
        for ix in range(minx, maxx):
            for iy in range(miny, maxy):
                ...
                if distance <= r:
                    ...
                    scalar_field[ix][iy] += int(metaball[i].D0) * math.exp((-100)*(distance**2))
...
def draw_2D_ScalarField(sf, min, max):
    ...
    for i in range(0, NX):
        ...
        for j in range(0, NY):
            ...
            index9 = int(255 * (sf[i][j] - min) / (max - min)) if sf[i][j] < max else 255

            ## specify the color to glColor3f using color_map ##
            glColor3f(color_map[index9][0], color_map[index9] [1], color_map[index9][2])
            ...
            index12 = int(255 * (sf[i][j+1] - min) / (max - min)) if sf[i][j+1] < max else 255

            ## specify the color to glColor3f using color_map ##
            glColor3f(color_map[index12][0], color_map[index12][1], color_map[index12][2])
