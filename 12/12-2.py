...
def search_cv2_cascade(castom_cascade_path="data"):
    ...
    if isdir(cascade_dir_path):
        ...
        cascade = {splitext(basename(path))[
            0][12:]: path for path in glob(cascade_xml_path_list)}
        return cascade
    else:
        return False
...
#     "/Users/keita/opt/anaconda3/lib/python3.8/site-packages/cv2/data/haarcascade_frontalface_default.xml")

eye_cascade = cv2.CascadeClassifier(cascade_files["eye"])

''' for debug '''
# eye_cascade = cv2.CascadeClassifier(
#     "/Users/keita/opt/anaconda3/lib/python3.8/site-packages/cv2/data/haarcascade_eye.xml")
...
for (x, y, w, h) in face:
    ...
    ### Search for the eyes in the face region roi_gray using detectMultiScale ###
    eyes = eye_cascade.detectMultiScale(roi_gray)
    for (ex, ey, ew, eh) in eyes:
        ### Draw the green rectangles on the eyes region ###
