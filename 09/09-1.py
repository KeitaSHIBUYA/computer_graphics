...
for i in range(height):
	for j in range(width):
		## Calculate a result value at (j, i) using a ternary operator and src_float ##
		result[i][j] = 255 if src_float[i][j] > 127 else 0
		# print(result)
		## Calculate an error value e from a result value and a src_float value at (j, i) ##
		e = src_float[i][j] - result[i][j]
		# print(e)
		if j != width - 1 and j != 0 and i != height - 1:
			## Calculate src_float value at f_1 in a slide ##
			src_float[i][j+1] += 5*e/16
			## Calculate src_float value at f_2 in a slide ##
			src_float[i+1][j-1] += 3*e/16
			## Calculate src_float value at f_3 in a slide ##
			src_float[i+1][j] += 5*e/16
			## Calculate src_float value at f_4 in a slide ##
			src_float[i+1][j+1] += 3*e/16
