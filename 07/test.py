...
class Birds:
    ...
    def update(self, index, dt):
        ...
        self.vel += 0.01*cohesion_rule(index, vl) + 0.09 * \
            alignment_rule(index, vl) + 0.9*separation_rule(index, vl)
        self.vel = 0.1 * self.vel / \
            np.linalg.norm(self.vel) if np.linalg.norm(
                self.vel) != 0 else self.vel
    ...
    def draw(self, timestep):
        ...
        draw_elipse(self.x, self.y, 0.03, 0.01,
                    math.atan2(self.vel[1], self.vel[0]))
        draw_wings(self.x, self.y, math.atan2(
            self.vel[1], self.vel[0]), timestep)
...
def draw_wings(x, y, angle, step):
    wing = [[0.01, 0.0], [0.01, 0.02], [-0.03, 0.03],
            [0., 0.01], [0., -0.01], [-0.03, -0.03], [0.01, -0.02]]
    tail = [[-0.02, 0.0], [-0.035, 0.015], [-0.035, -0.015]]
    beak = [[0.033, 0.0], [0.03, 0.002], [0.03, -0.002]]

    if step % 250 < 125:
        wing = [[0.01, 0.0], [0.01, 0.03], [-0.005, 0.015],
...
def cohesion_rule(bird_index, visible_list):
    ...
    vec_cohesion = average_pos - [bird[bird_index].x,
                                  bird[bird_index].y]  # calculate vec_cohesion ##
    return vec_cohesion
...
def alignment_rule(bird_index, visible_list):
    ...
    # calculate vec_alignment ##
    vec_alignment = average_vel - bird[bird_index].vel
...
def separation_rule(bird_index, visible_list):
    ...
    for i in range(len(visible_list)):
        distance = ((bird[visible_list[i]].x - main_x)**2 +
                    (bird[visible_list[i]].y - main_y)**2)**0.5
        if distance < R*0.2:
            # add a separation vector to sum_vel that is inversely proportional to the square of the distance ##
            sum_vel = distance / (distance**2)
    # let vec_separation be the vector by normalizing sum_vel ##
    vec_separation = sum_vel / \
        np.linalg.norm(sum_vel) if np.linalg.norm(sum_vel) != 0 else sum_vel
...
def main():
    ...
    for i in range(bird_num):
        b = Birds(random.random(), random.random(), np.array(
