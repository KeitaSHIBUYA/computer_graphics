...
def cohesion_rule(bird_index, visible_list):
    ...
    ## calculate vec_cohesion ##
    vec_cohesion = average_pos - np.array([bird[bird_index].x, bird[bird_index].y])
    # print(vec_cohesion)
    return vec_cohesion
...
def separation_rule(bird_index, visible_list):
    ...
    for i in range(len(visible_list)):
        ...
        if distance < R*0.2:
            ...
            sum_vel += np.array([(main_x - bird[visible_list[i]].x), (main_y - bird[visible_list[i]].y)]) / (distance ** 2)
            # sum_vel += distance / (distance ** 2)
    ## let vec_separation be the vector by normalizing sum_vel ##
    vec_separation = sum_vel / np.linalg.norm(sum_vel) if np.linalg.norm(sum_vel) != 0 else sum_vel
