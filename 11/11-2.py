...
def drawOptFlow(img, flow, step=16, dispsc=10):
    cimg    = img.copy()
    ###  Get the height h and width w from img  ###
    h, w, ch = img.shape
    y, x    = np.mgrid[step/2:h:step, step/2:w:step].reshape(2,-1).astype(int)
    dx, dy  = flow[y,x].T * dispsc
    ###  Calculate a length of optical flow vector and store it to dist  ###
...
###  Call drawOptFlow in specifying step=10 and substitute the results into rgb2  ###
rgb2 = drawOptFlow(frame2, flow, step=10, dispsc=10)
