...
def draw_sphere_shadow(N, M):
    ...
    for j in range(0, N):
        ...
        for i in range(0, M):
            ...
            p00 = np.array([math.sin(theta)*math.sin(phi),
                           math.cos(theta)+2, math.sin(theta)*math.cos(phi)])
            p01 = np.array([math.sin(prev_theta)*math.sin(phi),
                           math.cos(prev_theta)+2, math.sin(prev_theta)*math.cos(phi)])
            p11 = np.array([math.sin(prev_theta)*math.sin(prev_phi),
                           math.cos(prev_theta)+2, math.sin(prev_theta)*math.cos(prev_phi)])
            p10 = np.array([math.sin(theta)*math.sin(prev_phi),
                           math.cos(theta)+2, math.sin(theta)*math.cos(prev_phi)])

            normal = np.array([math.sin(theta) * math.sin(phi),
                              math.cos(theta), math.sin(theta) * math.cos(phi)])
            ...
            if np.dot(normal, p00 - light_pos) > 0:
                for k in range(len(light_array)):
                    ...
                    glBegin(GL_QUADS)
                    ## calculate intersection to xz plane using calculate_intersection_to_plane ##
                    sp00 = calculate_intersection_to_plane(p00, light_pos, 0.0)
                    glVertex3f(sp00[0], sp00[1], sp00[2])
                    ## calculate intersection to xz plane using calculate_intersection_to_plane ##
                    sp01 = calculate_intersection_to_plane(p01, light_pos, 0.0)
                    glVertex3f(sp01[0], sp01[1], sp01[2])
                    ## calculate intersection to xz plane using calculate_intersection_to_plane ##
                    sp11 = calculate_intersection_to_plane(p11, light_pos, 0.0)
                    glVertex3f(sp11[0], sp11[1], sp11[2])
                    ## calculate intersection to xz plane using calculate_intersection_to_plane ##
                    sp10 = calculate_intersection_to_plane(p10, light_pos, 0.0)
